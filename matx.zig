...
pub fn main() u8 {
    ...
    const uri = std.Uri.parse(blk: {
        ...
        defer args.deinit();
        // TODO: Output file name parameter.
        _ = args.skip();
        break :blk while (args.next()) |arg| switch (std.hash.Fnv1a_64.hash(arg)) { // TODO: Is 64-bit FNV-1a really the best hashing algorithm for the job?
    ...
    const json = blk: {
        ...
        var request = client.request(.GET, uri, headers, .{}) catch |err| {
...
inline fn render(registry : Registry) !void {
    var stdout_buffered = std.io.bufferedWriter(std.io.getStdOut().writer());
    defer stdout_buffered.flush() catch {}; // |err| stderr.print("Failed to flush stdout buffer: error \"{s}\"\n", .{ @errorName(err) }) catch {};
